...
class Day2Spec extends FlatSpec with Matchers {
  ...
  "example program" should "match" in {
    Day2Solution.run(0, Seq (1,0,0,0,99)) should be (Seq(2,0,0,0,99))
  }

  "example program2" should "match" in {
    Day2Solution.run(0, Seq (2,3,0,3,99)) should be (Seq(2,3,0,6,99))
  }

  "example program3" should "match" in {
    Day2Solution.run(0, Seq(2,4,4,5,99,0)) should be (Seq(2,4,4,5,99,9801))
  }

  "example program4" should "match" in {
    Day2Solution.run(0, Seq (1,1,1,4,99,5,6,0,99)) should be (Seq(30,1,1,4,2,5,6,0,99))
  }
