...
class Day9Spec extends FlatSpec with Matchers {
  ...
  "(from day5) example program from day 2" should "still work here" in {
    IntCodeComputer.start(Seq (1,0,0,0,99).map(_.toLong), 0).program.take(5) should be (Seq(2,0,0,0,99))
  }

  "(from day5) example program 2 from day 2" should "still work here" in {
    IntCodeComputer.start(Seq (2,3,0,3,99).map(_.toLong), 0).program.take(5) should be (Seq(2,3,0,6,99))
  }

  "(from day5) example program 3 from day 2" should "still work here" in {
    IntCodeComputer.start(Seq(2,4,4,5,99,0).map(_.toLong), 0).program.take(6) should be (Seq(2,4,4,5,99,9801))
  }

  "(from day5) example program 4 from day 2" should "still work here" in {
    IntCodeComputer.start(Seq (1,1,1,4,99,5,6,0,99).map(_.toLong), 0).program.take(9) should be (Seq(30,1,1,4,2,5,6,0,99))
  }
  ...
  "(from day5) consider whether the input is equal to 8" should "output 1 (if it is) or 0 (if it is not)." in {
    ...
    IntCodeComputer.start(program, 8).output should be (Seq(1))
    IntCodeComputer.start(program, 3).output should be (Seq(0))
    IntCodeComputer.start(program, 13).output should be (Seq(0))
  }
  ...
  "(from day5) consider whether the input is less than 8" should "output 1 (if it is) or 0 (if it is not)." in {
    ...
    IntCodeComputer.start(program, 1).output should be (Seq(1))
    IntCodeComputer.start(program, 3).output should be (Seq(1))
    IntCodeComputer.start(program, 8).output should be (Seq(0))
    IntCodeComputer.start(program, 18).output should be (Seq(0))
  }
  ...
  "(from day5) Using immediate mode, consider whether the input is equal to 8" should "output 1 (if it is) or 0 (if it is not)." in {
    ...
    IntCodeComputer.start(program, 1).output should be(Seq(0))
    IntCodeComputer.start(program, 8).output should be(Seq(1))
    IntCodeComputer.start(program, 10).output should be(Seq(0))
  }
  ...
  "(from day5) using position mode, some jump tests that take an input, then" should "output 0 if the input was zero or 1 if the input was non-zero:" in {
    ...
    IntCodeComputer.start(program, -10).output should be (Seq(1))
    IntCodeComputer.start(program, 0).output should be (Seq(0))
    IntCodeComputer.start(program, 10).output should be (Seq(1))
  }
  ...
  "(from day5) larger example" should "output 999 if the input value is below 8, output 1000 if the input value is equal to 8, or output 1001 if the input value is greater than 8." in {
    ...
    IntCodeComputer.start(program, 0).output should be(Seq(999))
    IntCodeComputer.start(program, -890).output should be(Seq(999))
    IntCodeComputer.start(program, 7).output should be(Seq(999))

    IntCodeComputer.start(program, 8).output should be(Seq(1000))

    IntCodeComputer.start(program, 9).output should be(Seq(1001))
    IntCodeComputer.start(program, 19).output should be(Seq(1001))
  }
  ...
  "example of self copy program" should "produce a copy of itself" in {
    ...
    IntCodeComputer.start(program,Seq.empty).output should be (program)
  }
  ...
  "1102,34915192,34915192,7,4,7,99,0" should " should output a 16-digit number" in {
    ...
    IntCodeComputer.start(program,Seq.empty).output.head.toString.length should be (16)
  }

  "104,1125899906842624,99" should "output the large number in the middle" in {
    val program = Seq (104L,1125899906842624L,99L)
    IntCodeComputer.start(program,Seq.empty).output.head should be (1125899906842624L)
  }
