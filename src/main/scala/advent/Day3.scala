...
object Day3Parts extends App {
  ...
  Using(Source.fromFile("src/main/data/day3-move-sequence.txt")) {
    file => {
      ...
      println(s"Advent of code 2019 - Day 3 / part 2: shortest timing to intersection: ${resultPart2}")   // 28580
    }
  }
  ...
  def closestIntersectionDistance(rawMoves1: String, rawMoves2: String): Int =
    Day3
      .intersections(
        Day3.path(Day3.parsedMoves(rawMoves1)),
        Day3.path(Day3.parsedMoves(rawMoves2)))
  ...
  def shortestIntersectionTiming(rawMoves1: String, rawMoves2: String): Int = {
    ...
    val path1 = Day3.path(Day3.parsedMoves(rawMoves1))
    val path2 = Day3.path(Day3.parsedMoves(rawMoves2))

    Day3
      .intersections(path1, path2)
      .diff(Set(Day3.centralPort))
      .map(intersection => path1.indexOf(intersection) + path2.indexOf(intersection))
      .reduce(Math.min)
  }
}
...
object Day3 {
  ...
  // seq of visited location when applying those moves from the central port
  def path(moves: Seq[Move]): Path =
    moves.foldLeft(Seq(centralPort)) { (locs, move) => locs ++ move.from(locs.last) }
